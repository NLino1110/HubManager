...
namespace ApiManager
{
    public class HubTarjetas
    {
        ...
        string EndPointApi = "/connect/get_tarjetas"; //Se espera que este sea el endpoint definitivo

        readonly RestSharpMiddle _client;
        private AppSession _appSession { get; }

        public HubTarjetas(AppSession _setAppSession)
        {
        ...
        public async Task<ApiResponseTarjetas?> GetAll()
        {
            ...
            {
                var resultApi = Newtonsoft.Json.JsonConvert.DeserializeObject<ApiResponseTarjetas>(result.Content);
                return resultApi;
            }
