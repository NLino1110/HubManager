...
namespace DMOrdersUI
{
    public partial class Login : ContentPage
    {
        ...
        {
            Debug.WriteLine("Cerrando login");
            base.OnDisappearing();
        }
        ...
        public async Task<bool> LoadSettingsFromDb()
        {
            ...
            App.Session.db_limit_default = await appSettingsDb.getInteger("db_limit_default");
            App.Session.sync_date_since = await appSettingsDb.getDateTime("sync_date_since");            
            ...
            if (resultUser != null)
            {
                if (resultUser.uid > 0)
                {
                    ...
                    ddCompany.SelectedItemChanged += async (s, e) =>
                    {
                        ...
                        StockWareHouseDb stockWareHouseDb = new StockWareHouseDb();
                        var warehouses = await stockWareHouseDb.GetItemsAsync(res_CompanySelected.id);

                        ddWarehouse.SelectedItem = null;
                        ddWarehouse.ItemsSource = warehouses;
                        ddWarehouse.ItemDisplayBinding = new Binding("name");
                        ddWarehouse.SelectedItem = warehouses[0];
                    };
        ...
        private void btnAccess_Clicked(object sender, EventArgs e)
        {
            ...
            if (ddCompany.SelectedItem != null && ddWarehouse.SelectedItem != null)
            {
                App.Session.res_Company = (res_company)ddCompany.SelectedItem;
