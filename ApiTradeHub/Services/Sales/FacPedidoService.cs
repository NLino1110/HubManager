...
namespace ApiTradeHub.Services.Sales
{
    public class FacPedidoService
    {
        ...
        public CajCajas GetFirstCajCajas(int? CodBodegaAgencia)
        {
            CajCajas returnObject = null;
            try
            {
                var query = _context.CAJCAJAS.AsQueryable();
                var BodegaAgencia = _context.BODEGASXAGENCIA.Where(sl => sl.CodBodegaAgencia == CodBodegaAgencia).FirstOrDefault();
                //TODO: Evaluar si se debe agregar una condicion donde el USUARIOCAJA tenga el texto "caja"
                returnObject = query.Where(sl => sl.CodAgencia == BodegaAgencia.CodAgencia).
                    OrderBy(o => o.NumCaja).FirstOrDefault();
            }
            catch (Exception e)
            {
