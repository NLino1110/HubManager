...
namespace ApiManager
{
    public class HubCartera
    {
        ...
        public HubCartera(AppSession _setAppSession)//(IOptions<AppSettings> appSettings)
        {
        ...
        /// <returns></returns>
        public async Task<ApiResponse_OBTENER_CARTERA_CAB?> ObtenerCabeceras(ApiRequest_v1 requestObject)
        {            
            string Action = "OBTENER_CARTERA_CAB";
            ...
            if (result != "")
            {
                var resultUser = Newtonsoft.Json.JsonConvert.DeserializeObject<ApiResponse_OBTENER_CARTERA_CAB>(result);
                return resultUser;
            }
