...
namespace ApiManager
{
    public class HubNotaCredito
    {
        ...
        public HubNotaCredito(AppSession _setAppSession)//(IOptions<AppSettings> appSettings)
        {
        ...
        /// <returns></returns>
        public async Task<ApiResponse_GUARDAR_NOTACREDITO> Send(ApiRequest_v1 requestObject)
        {            
            string Action = "GUARDAR_NOTACREDITO";
            ...
            if (result != "")
            {                
                var _resResponse = Newtonsoft.Json.JsonConvert.DeserializeObject<ApiResponse_GUARDAR_NOTACREDITO>(result);
                return _resResponse;
            }
