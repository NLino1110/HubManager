...
namespace ApiManager
{
    public class HubFactura
    {
        ...
        public HubFactura(AppSession _setAppSession)//(IOptions<AppSettings> appSettings)
        {
        ...
        /// <returns></returns>
        public async Task<ApiResponse_OBTENER_FAC_NOTACREDITO_CAB?> ObtenerCabeceras(ApiRequest_v1 requestObject)
        {            
            string Action = "OBTENER_FAC_NOTACREDITO_CAB";
            ...
            parameters.Add(RestSharp.Parameter.CreateParameter("codusuario", requestObject.codusuario, ParameterType.QueryString));
            ...
            if (result != "")
            {
                var resultUser = Newtonsoft.Json.JsonConvert.DeserializeObject<ApiResponse_OBTENER_FAC_NOTACREDITO_CAB>(result);
                return resultUser;
            }
        ...
        public async Task<ApiResponse_OBTENER_FAC_NOTACREDITO_DET?> ObtenerDetalles(ApiRequest_v1 requestObject)
        {
            string Action = "OBTENER_FAC_NOTACREDITO_DET";
            string currentDateTime = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
            //string cadenaJson = requestObject.cadenaJson;
            ...
            if (result != "")
            {
                var resultUser = Newtonsoft.Json.JsonConvert.DeserializeObject<ApiResponse_OBTENER_FAC_NOTACREDITO_DET>(result);
                return resultUser;
            }
